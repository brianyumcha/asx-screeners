"""Rebuilds industrials-index.html from scratch: reads the Industrials
...
renders the static page from industrials_index_template.html.
...
build_staples_index.py/build_discretionary_index.py (see
.github/workflows/materials_index.yml) as its own isolated job.

Like Real Estate/Financials/Staples/Consumer Discretionary, Industrials
has no natural explorer/producer or clinical-trial lifecycle - nearly
every ticker here is already a revenue-generating engineering, freight or
services business. This index skips Stage entirely and shows dividend
yield instead - the metric that actually matters for this sector.
"""
import datetime
import json
import time
...
CATEGORY_CACHE = "industrials_category_cache.json"
TEMPLATE = "industrials_index_template.html"
OUTPUT = "industrials-index.html"
