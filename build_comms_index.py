"""Rebuilds comms-index.html from scratch: reads the Communication
Services category classification HH SCREENER already maintains, fetches
a fresh name/market cap/price/yield snapshot for every ticker via
yfinance, and renders the static page from comms_index_template.html.

Runs on the same schedule as build_materials_index.py and its siblings
(see .github/workflows/materials_index.yml) as its own isolated job.

Like the other pass-through indexes, Communication Services has no
natural explorer/producer or clinical-trial lifecycle - nearly every
ticker here is already a revenue-generating telecom, media or ad-tech
business. This index skips Stage entirely and shows dividend yield
instead - the metric that actually matters for this sector.
"""
import datetime
import json
import time
...
CATEGORY_CACHE = "comms_category_cache.json"
TEMPLATE = "comms_index_template.html"
OUTPUT = "comms-index.html"
