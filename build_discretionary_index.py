"""Rebuilds discretionary-index.html from scratch: reads the Consumer
Discretionary category classification HH SCREENER already maintains,
fetches a fresh name/market cap/price/yield snapshot for every ticker via
yfinance, and renders the static page from discretionary_index_template.html.

Runs on the same schedule as build_materials_index.py/
build_healthcare_index.py/build_energy_index.py/build_tech_index.py/
build_real_estate_index.py/build_financials_index.py/
build_staples_index.py (see .github/workflows/materials_index.yml) as its
own isolated job.

Like Real Estate/Financials/Staples, Consumer Discretionary has no natural
explorer/producer or clinical-trial lifecycle - nearly every ticker here
is already a revenue-generating retailer, leisure or auto business. This
index skips Stage entirely and shows dividend yield instead - the metric
...
CATEGORY_CACHE = "discretionary_category_cache.json"
TEMPLATE = "discretionary_index_template.html"
OUTPUT = "discretionary-index.html"
