"""Rebuilds financials-index.html from scratch: reads the Financials
...
renders the static page from financials_index_template.html.

Runs on the same schedule as build_materials_index.py/
build_healthcare_index.py/build_energy_index.py/build_tech_index.py/
build_real_estate_index.py (see .github/workflows/materials_index.yml)
as its own isolated job.

Like Real Estate, Financials has no natural explorer/producer or
clinical-trial lifecycle - nearly every ticker here is already a
revenue-generating bank, insurer or asset manager. This index skips
Stage entirely and shows dividend yield instead - the metric that
actually matters for this sector.
"""
import datetime
import json
import time
...
CATEGORY_CACHE = "financials_category_cache.json"
TEMPLATE = "financials_index_template.html"
OUTPUT = "financials-index.html"
